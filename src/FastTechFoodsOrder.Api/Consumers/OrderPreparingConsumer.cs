...
namespace FastTechFoodsOrder.Api.Consumers
{
    public class OrderPreparingConsumer : IMessageHandler<OrderPreparingMessage>
    {
        ...
        public async Task HandleAsync(OrderPreparingMessage message, Activity? activity = null)
        {
            ...
            if (result.IsFailure)
            {
                _logger.LogError("Failed to update order status to PREPARING for OrderId: {OrderId}. Error: {Error}", 
                    message.OrderId, result.ErrorMessage);
                childActivity?.SetTag("operation.success", false);
                throw new Exception($"Failed to update order status to PREPARING for OrderId: {message.OrderId}. Error: {result.ErrorMessage}");
            }
