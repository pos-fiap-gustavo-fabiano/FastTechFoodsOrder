...
namespace FastTechFoodsOrder.Application.Services
{
    public class OutboxProcessorService : IHostedService, IDisposable
    {
        ...
        private async void ProcessOutboxEvents(object? state)
        {
            ...
            try
            {
                ...
                foreach (var outboxEvent in events)
                {
                    ...
                    {
                        // 1. Publica o evento
                        await PublishEvent(outboxEvent, rabbitMQPublisher);
        ...
        private async Task PublishEvent(Domain.Entities.OutboxEvent outboxEvent, IRabbitMQPublisher rabbitMQPublisher)
        {
            ...
            switch (outboxEvent.EventType)
            {
                case nameof(OrderCreatedMessage):
                    ...
                    if (orderCreated != null)
                    {
                        ...
                        await rabbitMQPublisher.PublishAsync(orderCreated, "order.created.queue");
                    }
                    break;
                ...
                //    {
                //        activity?.SetTag("order.id", orderPending.OrderId);
                //        await rabbitMQPublisher.PublishAsync(orderPending, "order.pending.queue");
                //    }
                //    break;
